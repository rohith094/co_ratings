...
const SubjectsUpload = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:3002/admin/addsubjects', formData, {
        headers: {
      ...
      setMessage('Failed to upload subjects. Please try again.');
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="w-full h-[90vh] overflow-y-scroll mx-auto p-4 bg-white rounded-lg">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        >
          {isLoading ? 'Uploading...' : 'Upload Subjects'}
        </button>
      </div>
